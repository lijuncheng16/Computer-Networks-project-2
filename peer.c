 ...
 *          Dave Andersen,
 * Edited by : Billy Li <junchenl@andrew.cmu.edu>
 *          	Yi Shi <yishi@andrew.cmu.edu>
...
char **retrieve_chunk_list(packet *incomingPacket) {
	...
	for (num_chunks = 0; num_chunks < incomingPacket->payload[0];
			num_chunks++) {
	...
	case ACK:
		...
		} else //normal
		{
			...
			if (incomingPacket.ack_num == BT_CHUNK_SIZE) {
				...
				{
					if (headRTT != NULL) {
						for (tempRtt = headRTT; tempRtt->next != NULL; tempRtt =
								tempRtt->next)
							;
